...
In \texttt{adegraphics}, a user-level function produces a plot that is stored (and returned) as an object. The class architecture of the objects created by \texttt{adegraphics} functions is described in Figure \ref{classes}. 
...
\begin{longtable}{|a|l|}
  \caption{\label{functionsADEgchanged} Changes in functions names between \texttt{ade4} and \texttt{adegraphics}}\\
      ...
      \hline \texttt{table.cont}, \texttt{table.dist}, \texttt{table.value} & \texttt{table.value} \footnote{The \texttt{table.value} function is now generic and can handle \texttt{dist} or \texttt{table} objects as arguments.} \\
      \texttt{table.paint} & \texttt{table.image}  \\
      \hline \texttt{sco.boxplot} & \texttt{s1d.boxplot}  \\
      \texttt{sco.class} & \texttt{s1d.class} \\
      \texttt{sco.distri} & \texttt{s1d.distri}  \\
      \texttt{sco.gauss} & \texttt{s1d.gauss}  \\
      \texttt{sco.label} & \texttt{s1d.label}  \\
      \texttt{sco.match} & \texttt{s1d.match}  \\
      \texttt{sco.quant} & no equivalence  \\      
      \hline \texttt{s.chull} & \texttt{s.class}\footnote{Convex hulls are now drawn by the \texttt{s.class} function (argument \texttt{chullSize})}\\
      \texttt{s.kde2d} & \texttt{s.density}  \\
      \texttt{s.match.class} & no equivalence  \\
      \hline \texttt{triangle.biplot} & \texttt{triangle.match}  \\
      \texttt{triangle.plot} & \texttt{triangle.label}  \\
...
<<chunk2, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
data(olympic)
...
<<chunk3, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
class(g1)
showClass("C1.barchart")
@

This object contains different slots:
<<chunk4, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
slotNames(g1)
@ 
...
<<chunk5, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
g1@data
@
...
<<chunk6, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
class(g2)
g2@g.args
@
...
For instance:
<<chunk7, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
...
<<chunk8, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
...
<<label=chunk9, include = FALSE, fig = FALSE, eval = FALSE, echo = TRUE, keep.source = FALSE, width = 5, height = 5>>=
g5
...
<<label=plot8, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 8, height = 8>>=
rbindADEg(cbindADEg(g2, g3), cbindADEg(g5, g6), plot = TRUE)
@
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot8.pdf}
\end{figure}
...
<<chunk10, include = FALSE, fig = FALSE, echo = TRUE, keep.source = TRUE>>=
length(g7)
names(g7)
...
<<label=plot18, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 8, height = 6>>=
...
<<label=plot19, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 8, height = 6>>=
...
<<label=plot22, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 6, height = 4>>=
...
<<label=plot26, fig = TRUE, include = FALSE, echo = TRUE, keep.source = FALSE, width=5, height=5>>=
adegpar("ppoints")
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{fig-plot26.pdf}
\end{figure}
...
<<label=plot27, fig = TRUE, include = FALSE, echo = TRUE, keep.source = FALSE, width=8, height=6>>=
s.label(pca2$li, facets = jv73$fac.riv, samelimits = FALSE)
@
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{fig-plot27.pdf}
\end{figure}


Several other \texttt{g.args} parameters can be updated according to the class of the created object (see Figure \ref{fig:2}).
...
<<label=plot28, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 8, height = 8>>=
...
<<plot30, fig=TRUE, include=FALSE, echo=TRUE, width=9, height=3>>=
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{fig-plot30.pdf}
\end{figure}
...
<<plot31, fig=TRUE, include=FALSE, echo=TRUE, width=9, height=3>>=
plot(tra1)
@

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{fig-plot31.pdf}
\end{figure}

Graphical parameters can be modified by:
<<plot32, fig=TRUE, include=FALSE, echo=TRUE, width=9, height=3>>=
plot(tra1, histDist.ppoly.col = "green", pbackground.col = "grey")
@

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{fig-plot32.pdf}
\end{figure}
...
<<label=plot33, fig = TRUE, include = FALSE, echo = TRUE, keep.source = FALSE, width=8, height=4>>=
data(meaudret)
...
<<label=plot34, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 8, height = 8>>=
...
<<label=plot35, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 8, height = 8>>=
data(rpjdl)
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot35.pdf}
\end{figure}


\subsection{1-D plot}

<<label=plot36, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 8, height = 8>>=
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot36.pdf}
\end{figure}


\subsection{Maps and neighbor graphs}

<<label=plot37, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 10, height = 5>>=
library(Guerry)
library(sp)
data(gfrance85)
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot37.pdf}
\end{figure}


<<label=plot38, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 5, height = 4>>=
s.Spatial(gfrance85[,7:12])
@
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{fig-plot38.pdf}
\end{figure}


<<label=plot39, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 8, height = 4>>=
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot39.pdf}
\end{figure}


\subsection{Ternary plots}

<<label=plot40, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 8, height = 4>>=
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot40.pdf}
\end{figure}
...
\begin{longtable}{|p{3cm}|p{7cm}|p{4cm}|p{4cm}|p{4cm}|}
  ...
  \hline \texttt{addaxes} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{triangle.class}, \texttt{triangle.plot} & & \texttt{paxes.draw} &\\
  \hline \texttt{area} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value} & \texttt{Sp} & & a \texttt{Sp} object \\
  \hline \texttt{axesell} & \texttt{s.class}, \texttt{s.distri}, \texttt{triangle.class} & & \texttt{pellipses.axes.draw} &\\
  \hline \texttt{box} & \texttt{s.corcircle}, \texttt{triangle.plot} &  & \texttt{pbackground.box} &\\
  \hline \texttt{boxes} & \texttt{s.arrow}, \texttt{s.label}, \texttt{sco.class}, \texttt{sco.label}, \texttt{sco.match} &  & \texttt{plabels.boxes.draw} &\\ 
  \hline \texttt{cellipse} & \texttt{s.class}, \texttt{s.distri}, \texttt{triangle.class} & \texttt{ellipseSize} & & \\
  \hline \texttt{cgrid} & \texttt{s.arrow}, \texttt{s.class}, \texttt{s.chull}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{pgrid.nint} & both play on the grid mesh, but they are not strictly equivalent\\
  \hline \texttt{clabel} & \texttt{s.arrow}, \texttt{s.class}, \texttt{s.chull}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.match}, \texttt{s.traject}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match}, \texttt{triangle.plot} & & \texttt{plabels.cex} & \\ 
  \hline \texttt{clabel} & \texttt{table.dist} & & & \texttt{axis.text = list()}, \texttt{lattice} parameter\\
  \hline \texttt{clabel.col} & \texttt{table.cont}, \texttt{table.paint}, \texttt{table.value} & & & \texttt{axis.text = list()}, \texttt{lattice} parameter\\ 
  \hline \texttt{clabel.row} & \texttt{table.cont}, \texttt{table.paint}, \texttt{table.value} & & & \texttt{axis.text = list()}, \texttt{lattice} parameter\\ 
  ...
  \hline \texttt{clegend} & \texttt{table.paint} & & \texttt{plegend.size} &\\
  \hline \texttt{clogo} & \texttt{s.logo} & & \texttt{ppoints.cex} &\\
  \hline \texttt{cneig} & \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.value} &  & \texttt{pnb.edge.lwd} &\\
  \hline \texttt{col.labels} & \texttt{table.cont}, \texttt{table.paint}, \texttt{table.value} & \texttt{labelsy} & & \\   
  \hline \texttt{contour} & \texttt{s.arrow}, \texttt{s.class}, \texttt{s.chull}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value} & \texttt{Sp} & & a \texttt{Sp} object \\
  \hline \texttt{contour.plot} & \texttt{s.image} & \texttt{region} & & \\
  \hline \texttt{cpoints}, \texttt{cpoint} & \texttt{s.arrow}, \texttt{s.class}, \texttt{s.chull}, \texttt{s.distri}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.class}, \texttt{sco.label}, \texttt{sco.match}, \texttt{triangle.class}, \texttt{triangle.plot} &  & \texttt{ppoints.cex} &\\
  \hline \texttt{csize} & \texttt{s.value}, \texttt{table.cont}, \texttt{table.dist}, \texttt{table.paint}, \texttt{table.value} & \texttt{ppoints.cex} & & \\
  \hline \texttt{csize} & \texttt{sco.distri} & \texttt{sdSize} & & \\ 
  \hline \texttt{cstar} & \texttt{s.class}, \texttt{s.distri}, \texttt{triangle.class} & \texttt{starSize} & & \\
  \hline \texttt{csub} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match}, \texttt{triangle.class}, \texttt{triangle.plot} & & \texttt{psub.cex} &\\
  \hline \texttt{draw.line} & \texttt{triangle.biplot}, \texttt{triangle.class}, \texttt{triangle.plot} & & \texttt{pgrid.draw} &\\
  \hline \texttt{edge} & \texttt{s.arrow}, \texttt{s.match}, \texttt{s.traject} & & \texttt{parrows.length} & setting the length of the arrows to 0 is equivalent to \texttt{edge = FALSE}\\
  \hline \texttt{grid} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match}, \texttt{table.cont}, \texttt{table.dist}, \texttt{table.value} & & \texttt{pgrid.draw} &\\  
  \hline \texttt{horizontal} & \texttt{sco.class}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{p1d.horizontal} &\\
  \hline \texttt{image.plot} & \texttt{s.image} & \texttt{contour} & & \\ 
  \hline \texttt{includeorigin} \texttt{include.origin} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{porigin.include} &\\
  \hline \texttt{kgrid} & \texttt{s.image} & \texttt{gridsize} & & \\
  \hline \texttt{klogo} & \texttt{s.logo} & & & no correspondence \\
  \hline \texttt{labeltriangle} & \texttt{triangle.class}, \texttt{triangle.plot} & & & no correspondance  \\
  \hline \texttt{legen} & \texttt{sco.gauss} & \texttt{labelplot} & & \\ 
  \hline \texttt{neig} & \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.value} &  \texttt{nbobject} & & a \texttt{nb} object\\
  \hline \texttt{optchull} & \texttt{s.chull} & \texttt{chullSize} & & \\ 
  \hline \texttt{origin} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{porigin.origin} &\\
  \hline \texttt{pch} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.distri}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.label}, \texttt{sco.match}, \texttt{triangle.class}, \texttt{triangle.plot}, \texttt{table.cont} &  & \texttt{ppoints.pch} &\\ 
  \hline \texttt{pixmap} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value} & & & no correspondance \\
  \hline \texttt{pos.lab} & \texttt{sco.class}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{p1d.labpos} &\\
  \hline \texttt{possub} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.class}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match}, \texttt{triangle.class}, \texttt{triangle.plot} & & \texttt{psub.pos} &\\
  \hline \texttt{rectlogo} & \texttt{s.logo} & \texttt{rect} & & \\
  \hline \texttt{reverse} & \texttt{sco.class}, \texttt{sco.gauss}, \texttt{sco.label}, \texttt{sco.match} & & \texttt{p1d.reverse} &\\
  \hline \texttt{row.labels} & \texttt{table.cont}, \texttt{table.paint}, \texttt{table.value} & \texttt{labelsx} & & \\
  \hline \texttt{scale} & \texttt{triangle.class}, \texttt{triangle.plot} & \texttt{adjust} & & \\
  \hline \texttt{show.position} & \texttt{triangle.class}, \texttt{triangle.plot} & \texttt{showposition} & & \\ 
  \hline \texttt{sub} & \texttt{s.arrow}, \texttt{s.chull}, \texttt{s.class}, \texttt{s.corcircle}, \texttt{s.distri}, \texttt{s.image}, \texttt{s.kde2d}, \texttt{s.label}, \texttt{s.logo}, \texttt{s.match}, \texttt{s.traject}, \texttt{s.value}, \texttt{sco.boxplot}, \texttt{sco.class}, \texttt{sco.distri}, \texttt{sco.gauss}, \texttt{sco.label} \texttt{sco.match}, \texttt{triangle.class}, \texttt{triangle.plot} & & \texttt{psub.text} &\\
