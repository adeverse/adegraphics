...
\begin{longtable}{|a|p{3cm}|p{8cm}|}
  \caption{\label{functionsADEg} Graphical functions available in \texttt{adegraphics}}\\
      ...
      \texttt{triangle.match} & \texttt{Tr.match} & Ternary plot of the matching between two sets of coordinates \\ \hline
   \end{longtable}
...
<<label=plot18, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 6, height = 8>>=
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot18.pdf}
\end{figure}
...
<<label=plot19, include = FALSE, fig = TRUE, echo = TRUE, keep.source = TRUE, width = 6, height = 8>>=
s.class(pca2$li, fac = jv73$fac.riv, col = rainbow(12), facets = jv73$fac.riv)
@

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot19.pdf}
\end{figure}
...
The full list of available parameters is summarized in Figure~\ref{fig:3}. The ordinate represents the different sublists and the abscissa gives the name of the parameters available in each sublist. Note that some row names have two keys separated by a dot: the first key indicates the first level of the sublist, etc. For example \texttt{plegend.text} is the sublist \texttt{text} of the sublist \texttt{plegend}. The parameters \texttt{col} and \texttt{cex} in \texttt{plegend.text} allow to control the aspect of the text of the legend.
...
<<label=plot25c, fig = TRUE, include = FALSE, echo = TRUE, keep.source = FALSE, width=6, height=8>>=
s.label(pca2$li, facets = jv73$fac.riv, samelimits = FALSE)
@
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig-plot25c.pdf}
\end{figure}
...
\subsection{Parameters applied on a ADEgS}
...
g23 <- s.value(coa2$li, coa2$li[,3], method = "color", maxsize = 0.8, plegend.text.cex = 0.8, plot = FALSE)
g24 <- s.value(coa2$li, coa2$li[,3], plegend.text.cex = 0.8, maxsize = 0.8, symbol = "square", method = "color", plegend.horizontal = FALSE, plegend.text.cex = 0.8, col = colorRampPalette(c("yellow", "blue"))(6), plot = FALSE)
g25 <- s.value(coa2$li, coa2$li[, 3], center = 0, method = "size", maxsize = 0.6, symbol = "circle", col = c("yellow", "red"), plot = FALSE)
...
g29 <- s1d.density(score1, fac1, col = c(2, 4), ppolygons.alpha = 0.2, p1d = list(rug = list(ticksize = 1, line = FALSE)), plot = FALSE)
...
<<label=plot32b, include = FALSE, fig = TRUE, echo = TRUE, keep.source = FALSE, width = 4, height = 6>>=
s.Spatial(gfrance85[,7:12])
@
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{fig-plot32b.pdf}
\end{figure}
...
\begin{longtable}{|p{3cm}|p{7cm}|p{4cm}|p{4cm}|p{4cm}|}
  ...
  \hline \texttt{clegend} & \texttt{s.value}, \texttt{table.cont}, \texttt{table.value} & & \texttt{plegend.text.cex} \texttt{plegend.hspace} \texttt{plegend.vspace} & various parameters setting the legend size\\
  \hline \texttt{clegend} & \texttt{table.image} & & \texttt{ppoints.cex} &\\
  ...
  \hline \texttt{csize} & \texttt{s.value} & \texttt{maxsize} & & \\
