...
scatter.dudi(dd1, posieig = "bottomright")
scatter.dudi(dd1, posieig = "bottomright", plot = T, prop = TRUE)
scatter.dudi(dd1, posieig = "none", plot = T)
scatter.dudi(dd1, posieig = "bottomleft", plot = T)
scatter.dudi(dd1, posieig = "topright", plot = T)
scatter.dudi(dd1, posieig = "topleft", plot = T, eig.col = c("white", "blue", "red"))

data(rhone, package = "ade4")
dd1 <- ade4::dudi.pca(rhone$tab, nf = 4, scannf = FALSE)
g1 <- scatter.dudi(dd1, sub = "Principal component analysis", row = list(plabels.optim = TRUE), col.pla.boxes.alpha = 0.5)
g1[2, drop = TRUE]
scatter.dudi(dd1, row = list(sub = "Principal component analysis", plabels.optim = TRUE), col.pla.boxes.alpha = 0.5)
scatter.dudi(dd1, prop = TRUE, ppoints.cex = 0.2, density.plot = TRUE, row = list(threshold = 0.01))
...
#scatter.dudi(analyF, xax = 1, yax = 2, prop = TRUE)
...
ade4::scatter.coa(dd2, method = 1, sub = "1 / Standard", posieig = "none")
ade4::scatter.coa(dd2, method = 2, sub = "2 / Columns -> averaging -> Rows", posieig = "none")
ade4::scatter.coa(dd2, method = 3, sub = "3 / Rows -> averaging -> Columns ", posieig = "none")
g1 <- scatter.coa(dd2, method = 1, row.sub = "1 / Standard", posieig = "none", plot = FALSE)
g2 <- scatter.coa(dd2, method = 2, col.sub = "2 / Columns -> averaging -> Rows", posieig = "none", plot = FALSE)
g3 <- scatter.coa(dd2, method = 3, row.sub = "3 / Rows -> averaging -> Columns ", posieig = "none", plot = FALSE)
