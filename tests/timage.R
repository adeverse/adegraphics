...
x <- rank(coa1$co[, 1])
y <- rank(coa1$li[, 1])
g4 <- table.image(Y, x = x, y = 1:8, axis.text = list(alpha = 0), pgrid.col = "black", pgrid.lwd = 0.8, col = c("white", "black"), plegend.draw = FALSE)
g5 <- table.image(X, x = x, y = y, ptable = list(x = list(tck = 0), y = list(tck = 4)), pleg.draw = FALSE, labelsy = paste(" ", row.names(X), sep = ""))
