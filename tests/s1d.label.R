...
g3 <- s1d.label(envpca$l1[, 1], row.names(envpca$l1), plabels.boxes.draw = FALSE, plab.ori = 45, plabel.boxes =  list(draw = FALSE))
g4 <- s1d.label(envpca$co[, 1], row.names(envpca$co), p1d.reverse = TRUE, poslabel = "value")
g5 <- s1d.label(envpca$l1[, 1], row.names(envpca$l1), p1d.lab = 0, plabel.cex = 0)
