...
g2 <- table.value(tab1, x = c(1:5, 10:6))
g3 <- table.value(tab1, x = c(1:5, 10:8))
...
g11 <- table.value(tab3, x = coa1$c1[, 1], y = coa1$l1[, 1], meanX = TRUE,  ablineX = TRUE)
g12 <- table.value(tab3, x = coa1$c1[, 1], y = coa1$l1[, 1], meanY = TRUE,  ablineY = TRUE)
...
g14 <- table.value(tab4, x = coa2$c1[, 1], y = rank(coa2$l1[, 1]),  axis.text = list(cex = 0), labelsy = rpjdl$lalab, plot = F)
